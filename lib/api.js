...
export const analyzePetPose = async (file, maxPets = 4) => {
  const body = new FormData();
  body.append("file", file);
  return requestJson(`/pose?max_pets=${maxPets}&threshold=0.2`, {
    method: "POST",
    body,
  });
};

export const analyzePetVideo = async (file, topK = 8) => {
  const body = new FormData();
  body.append("file", file);
  const payload = await requestJson(`/analyze/video?top_k=${topK}&max_pets=4`, {
    method: "POST",
    body,
  });
  return {
    ...
    tracks: (payload.tracks || []).map((track) => ({
      ...track,
      matches: mapRankedIdsToPhotos(track.results || []),
    })),
  };
};
